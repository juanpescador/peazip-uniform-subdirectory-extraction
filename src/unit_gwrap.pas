unit Unit_gwrap;
{
 DESCRIPTION     :  Unit launching 7za/7z console application, providing GUI for:
...
uses
  ...
  StdCtrls, Buttons, Process, Menus,
...
var
  ...
  Color4,Color5,caption_build,delimiter,confpath,peazippath,in_name,dummytitle:utf8string;
...
Form_gwrap.l4.Hint:='';
case modeofuse of
   1 : s1:=txt_5_3_test+' '+uextractfilename(in_name);
   4 : s1:=txt_5_3_info+' '+uextractfilename(in_name);
   5 : s1:=txt_5_3_list+' '+uextractfilename(in_name);
   end;
case modeofuse of
   3 :
   begin
   ...
   {$IFDEF MSWINDOWS}if length(s2) = 2 then else{$ENDIF} s2:=uextractfilename(s2);
...
if optype=0 then
   ...
   {$IFDEF MSWINDOWS}if length(s) = 2 then else{$ENDIF} s:=uextractfilename(s);
...
procedure displayinfo(exit_code:integer; exit_string:utf8string);
var
   s,s1,dummystr:utf8string;
   i,rc:integer;
...
//presentation
s:=uextractfilename(in_name)+char($0A)+char($0D)+char($0A)+char($0D)+s1+char($0A)+char($0D)+txt_5_3_details+char($0A)+char($0D)+'------'+char($0A)+char($0D)+s;
end
else s:=exit_string;
