...
{$ENDIF}
  ...
  list_utils, pea_utils, rfs_utils, ansiutf8_utils, unit_report, types;
...
const
  P_RELEASE          = '0.49'; //declares release version for the whole build
...
var
   ...
   i,j,k,errors,dfiles,ddirs,numread,numwritten,nlevel,nleveli,rc,progressive,attr:integer;
   buf:array[0..65535]of byte;
   aes_ctx:TAESContext;
   aes_iv:array[0..15]of byte;
   randomstring,randomstring2,oldrandomstring,in_param,s:UTF8string;
   sr:TSearchRec;
...
if (level='NONE') or (level='QUICK') or (level='RECYCLE') then
   Form_pea.LabelTools2.Caption:=inttostr(paramcount-2)+' items(s) sent to '+level+' deletion'
else
...
case level of
   ...
   begin
   Form_pea.LabelTools2.Caption:='Processing drive '+envtoutf8(paramstr(3))+', '+nicenumber(inttostr(sizetotal))+', pass '+inttostr(j+nlevel-nleveli)+' of '+inttostr(nlevel);
   tok:=false;
   gtotal:=0;
   n:=0;
   repeat
   ...
   until tok=true;
   recoverfreespace(n);
   Form_pea.LabelTools2.Caption:='Done drive '+envtoutf8(paramstr(3))+', '+nicenumber(inttostr(sizetotal))+', pass '+inttostr(j+nlevel-nleveli)+' of '+inttostr(nlevel);
   end;
   end;
end;
...
MessageDlg(envtoutf8(paramstr(2))+' is not accessible (or not a file)', mtError, [mbOK], 0);
Application.Terminate;
end;
if sizea>16*1024*1024 then
  begin
  MessageDlg('Hex preview is currently limited to small files, up to 16MB', mtWarning, [mbOK], 0);
  exit;
  end;
...
if EditPW1.Text='' then
   begin
   MessageDlg('Cannot accept empty password, please type it in "Passphrase" field', mtWarning, [mbOK], 0);
   exit;
   end
else
...
case ComboBoxUtils.ItemIndex of
   ...
   1: cl:=bin_name+' CHECK HEX MD5 ON '+in_param;
   2: cl:=bin_name+' CHECK HEX RIPEMD160 ON '+in_param;
   3: cl:=bin_name+' CHECK HEX SHA1 ON '+in_param;
   4: cl:=bin_name+' CHECK HEX SHA256 ON '+in_param;
   5: cl:=bin_name+' CHECK HEX ADLER32 CRC16 CRC24 CRC32 CRC64 EDONKEY MD5 RIPEMD160 SHA1 SHA256 ON '+in_param;
   6: cl:=bin_name+' CHECK HEX ALL ON '+in_param;
   7: begin
  ...
  10: begin
      for i:=0 to ListMemo.Lines.Count do
         if length(ListMemo.Lines[i])>1 then
            begin
            in_param:=delimiter+ListMemo.Lines[i]+delimiter;
            cl:=bin_name+' LIST INFO '+in_param;
...
LabelUtilsInput.Enabled:=True;
LabelOpenFile0.Enabled:=True;
LabelOpenFile2.Enabled:=True;
LabelOpenFile3.Enabled:=True;
...
LabelUtilsInput.Enabled:=False;
LabelOpenFile0.Enabled:=False;
LabelOpenFile2.Enabled:=False;
LabelOpenFile3.Enabled:=False;
ButtonUtilsReset.Enabled:=False;
if ComboBoxUtils.ItemIndex<>11 then
   begin
...
if paramcount>0 then
   ...
   if upcase(paramstr(1))='PEAUTILS' then
      ...
      Form_pea.PanelUtils.visible:=false;
      end;
   end
